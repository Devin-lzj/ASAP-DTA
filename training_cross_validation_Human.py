...
datasets = [['Human'][0]]
modeling = [ASAPNet_GLOBALLE,ASAPNet_HIERLE][0]
...
for fold in range(1, 6):
    ...
    if ((not os.path.isfile(processed_data_file_train)) or
        ...
        print('please run prepare_data_Human in pytorch format!')
    else:
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if valid_roc > best_roc:
                ...
                print('AUROC improved at epoch ', best_epoch, '; test_auc:{:.5f}'.format(test_roc), '; test_prc:{:.5f}'.format(test_prc),
                      '; test_precision:{:.5f}'.format(test_precision), '; test_recall:{:.5f}'.format(test_recall), model_st, dataset)
            else:
                print('No improvement since epoch ', best_epoch, '; test_auc:{:.5f}'.format(test_roc), '; test_prc:{:.5f}'.format(test_prc),
                      '; test_precision:{:.5f}'.format(test_precision), '; test_recall:{:.5f}'.format(test_recall), model_st, dataset)
