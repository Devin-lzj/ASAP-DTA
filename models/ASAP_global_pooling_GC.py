...
from torch_geometric.nn import ASAPooling,LEConv,GraphConv, GCNConv, SAGEConv, global_mean_pool as gap, global_max_pool as gmp



# 3DGCN model
class ASAPNet_GLOBALGC(torch.nn.Module):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128, num_features_xd=78, num_features_xt=25, output_dim=128, dropout=0.1):

        super(ASAPNet_GLOBALGC, self).__init__()
        ...
        self.pool1 = ASAPooling(in_channels=3*num_features_xd, ratio=self.pooling_ratio, GNN=GraphConv)
