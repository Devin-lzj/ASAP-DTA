...
from torch_geometric.nn import LEConv, GCNConv,ASAPooling, SAGEConv, GATConv, global_max_pool as gmp, global_mean_pool as gap



# 3DGCN model
class ASAPNet_HIERGCN(torch.nn.Module):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128, num_features_xd=78, num_features_xt=25, output_dim=128, dropout=0.1):

        super(ASAPNet_HIERGCN, self).__init__()
        ...
        self.pool1 = ASAPooling(in_channels=num_features_xd, ratio=self.pooling_ratio, GNN=GCNConv)


        self.conv2 = GCNConv(num_features_xd, num_features_xd)
        self.bn2 = nn.BatchNorm1d(num_features_xd)
        self.pool2 = ASAPooling(num_features_xd, ratio=self.pooling_ratio, GNN=GCNConv)  # ratio本身可理解为另一种形式的filter数

        self.conv3 = GCNConv(num_features_xd, num_features_xd)
        self.bn3 = nn.BatchNorm1d(num_features_xd)
        self.pool3 = ASAPooling(num_features_xd, ratio=self.pooling_ratio, GNN=GCNConv)
